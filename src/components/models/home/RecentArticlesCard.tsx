    ...
    <div ref={animateOnScroll}>
      <Card class="motion-safe:animate-pop-in-up animate-duration-300 animate-fill-forwards child-animate">
        ...
        <div class="flex flex-col gap-2">
          <For each={articles()?.slice(0, 5)}>
            {(article, index) => (
              <ArticleSubCard
                article={article}
                class="motion-safe:animate-pop-in-up animate-duration-300 child-animate animate-fill-forwards"
        ...
        <Show when={(articles()?.length ?? 0) > 5}>
          <a
            class="flex justify-center items-center text-sm text-gray-400 hover:text-white transition-colors duration-200 border-dashed border px-4 py-2 border-gray-500 mt-4 rounded hover:bg-white/10 font-medium motion-safe:animate-pop-in-up animate-duration-300 animate-delay-1400 invisible animate-fill-forwards"
            href="/articles"
          >
